...
const referenceImage = {
  base64:
    ...
    fs.readFileSync("client/assets/q1/p1.png", { encoding: "base64" }),
};
...
const refImgs = {
  q1: {
    ...
    p2: {
      base64:
        "data:image/png;base64," +
        fs.readFileSync("client/assets/q1/p2.png", { encoding: "base64" }),
    },
    p3: {
      base64:
        "data:image/png;base64," +
        fs.readFileSync("client/assets/q1/p3.png", { encoding: "base64" }),
    },
  },
  q2: {
    p1: {
      base64:
        "data:image/png;base64," +
        fs.readFileSync("client/assets/q2/p1.png", { encoding: "base64" }),
    },
    p2: {
      base64:
        "data:image/png;base64," +
        fs.readFileSync("client/assets/q2/p2.png", { encoding: "base64" }),
    },
    p3: {
      base64:
        "data:image/png;base64," +
        fs.readFileSync("client/assets/q2/p3.png", { encoding: "base64" }),
    },
  },
  q3: {
    p1: {
      base64:
        "data:image/png;base64," +
        fs.readFileSync("client/assets/q3/p1.png", { encoding: "base64" }),
    },
  },
};
...
async function compare(img1, img2) {
  return new Promise(async function (resolve, reject) {
    try {
      ...
      //   //r,g,b,a
      //   if (
      //     Math.abs(refImageData[i] - ImageData[i]) <= tone &&
      //     Math.abs(refImageData[i + 1] - ImageData[i + 1]) <= tone &&
      //     Math.abs(refImageData[i + 2] - ImageData[i + 2]) <= tone
      //   ) {
      //     count++;
      //   }
      // }
      ...
      //   __dirname + "/client/assets/referenceImage.png"
      // );
...
app.post("/getImage", async (req, res) => {
  ...
  let queNum = req.body.queNum;
  let imgIndex = req.body.imgIndex;
  ...
  for (let i = 0; i < forbiddenKeywords.length; i++) {
    ...
    if (transformedHTML.includes(thisKeyword)) {
      res.json({
        status: "error",
        message: `You can'tthe keyword "${thisKeyword}"`,
        code: 400 + i,
      });
      return;
    }
  ...
  try {
    ...
    compare(refImgs[queNum][imgIndex].base64, base64)
      .then(function (x) {
        if (x > 0.995) {
          res.json({
            status: "ok",
